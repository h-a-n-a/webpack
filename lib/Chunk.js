...
class Chunk {
	 ...
	 */
	addGroup(chunkGroup) {
		this._groups.add(chunkGroup); // add this chunk to the given ChunkGroup
	}
	...
	getAllReferencedChunks() {
		const queue = new Set(this.groupsIterable); // get referenced chunk groups of this chunk
		const chunks = new Set();
		// get chunks from chunk groups that referenced in this chunk
		for (const chunkGroup of queue) {
			for (const chunk of chunkGroup.chunks) {
				chunks.add(chunk);
			}

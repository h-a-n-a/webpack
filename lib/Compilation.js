...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	...
	_processModuleDependencies(module, callback) {
		...
		// create module creation job if for each newly discovered unique dependencies(参考 cacheKey, 看起来是在当前 module 中的 unique dependency identifier 判断)
		...
		const processDependencyForResolving = dep => {
			// Identifier for dependency, dependency like `HarmonyExportExpressionDependency` does not have an identifier
			...
			if (resourceIdent !== undefined && resourceIdent !== null) {
				...
				// 只在 cacheKey 不同的时候创建新的 dependency
				if (list === undefined) {
	...
	_handleModuleBuildAndDependencies(originModule, module, recursive, callback) {
		...
		this.buildModule(module, err => {
			...
			// process module dependencies after module's `handleParseResult` in module build
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
	...
	_codeGenerationModule(
		...
		cache.get((err, cachedResult) => {
			...
			for (const runtime of runtimes) {
				results.add(module, runtime, result); // add code generation result to results
			}
			if (!cachedResult) {
	...
	processRuntimeRequirements({
		...
		// get runtime requirements of each module and add them to the corresponding ChunkGraphModule
		for (const module of modules) {
			if (chunkGraph.getNumberOfModuleChunks(module) > 0) {
				...
				for (const runtime of chunkGraph.getModuleRuntimes(module)) {
					...
					const runtimeRequirements =
						codeGenerationResults.getRuntimeRequirements(module, runtime); // extract runtime requirements from code generation results, this is single module level, and does not merge the runtimeRequirements with other modules
					...
					additionalModuleRuntimeRequirements.call(module, set, context); // user can add additional runtime based on the runtime requirements by each module

					for (const r of set) {
						const hook = runtimeRequirementInModule.get(r); // add additional runtimeRequirements to `set` based on runtimeRequirements
						if (hook !== undefined) hook.call(module, set, context); // run the runtimeRequirementInModule hook (mostly registered via RuntimePlugin), this is used to add additional runtime requirements based on the runtime requirements of each module
					}
					if (set.size === 0) {
						if (memCache) {
							memCache.set(
						...
						if (memCache) {
							...
							chunkGraph.addModuleRuntimeRequirements(module, runtime, set); // store runtimeRequirements to the ChunkGraphModule of module
						}
					}
				}
			}
		...
		// merge module runtime requirements to chunk runtime requirements
		for (const chunk of chunks) {
			const set = new Set();
			for (const module of chunkGraph.getChunkModulesIterable(chunk)) {
				const runtimeRequirements = chunkGraph.getModuleRuntimeRequirements(
					// get module runtimeRequirements from ChunkGraphModule that stored in previous step
					module,
					chunk.runtime // the requested chunk runtime, e.g `main`
				);
				for (const r of runtimeRequirements) set.add(r); // merge all runtimeRequirements of modules in the chunk to `Set`
			}
			...
			chunkGraph.addChunkRuntimeRequirements(chunk, set); // store runtimeRequirements to the ChunkGraphChunk
		}
		...
		for (const treeEntry of chunkGraphEntries) {
			...
			for (const chunk of treeEntry.getAllReferencedChunks()) {
				const runtimeRequirements =
					chunkGraph.getChunkRuntimeRequirements(chunk); // get chunk runtimeRequirements from ChunkGraphChunk that stored in previous step

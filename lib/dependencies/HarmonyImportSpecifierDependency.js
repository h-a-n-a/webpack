...
HarmonyImportSpecifierDependency.Template = class HarmonyImportSpecifierDependencyTemplate // replace used identifiers with importVar
	extends HarmonyImportDependency.Template
{
	...
	apply(dependency, source, templateContext) {
		...
		const ids = dep.getIds(moduleGraph); // get imported ids TODO:(H) figure out whether the id comes with `import { a,b,c } from "./"` ["a","b","c"], since the namespace import does not have any ids
		const exportExpr = this._getCodeForIds(dep, source, templateContext, ids);
		const range = dep.range;
		if (dep.shorthand) {
			// if property is shorthanded
